...
int igra(char* abeceda, char* kopija_abecede, int kraj, int br, int duljina, int* pogodeno, char* odgovor, int zastavica) {
    while (!kraj) {
        ...
        if (pokusaj == '!') {
            ...
            file.write((char*)odgovor, duljina);
            file.write((char*)pogodeno, duljina);
            file.close();
        ...
        if (kopija_abecede[slovo] == '-' && pokusaj != '?')
            ...
            //Dio programa koji u slucaju naseg pisanja znaka "?" prikazuje ispisana sva neiskoristena slova
            if (pokusaj == '?') {
                cout << "Dostupna slova: ";
                for (int i = 0; i < 22; i++) {
                    if (kopija_abecede[i] != '-')
            ...
            //Ispis u svakon od mogucih slucajeva kada pogodimo krivo slovo
            if (zastavica == 0) {
                if (pokusaj != '?' && pokusaj != '!') {
                    br--;
                }
            ...
            cout << "\n";
            //Dio koda koji povezan uz gornje case naredbe crta hangmana
            switch (br)
            {
            case 0:
                cout << "  _____\n";
...
int main() {
    ...
    if (toupper(izbor) == 'Y') {
        ...
        int duljina = 128;
        file.read((char*)&odgovor, duljina);
        int* pogodeno = new int[duljina];
        file.read((char*)&pogodeno, duljina);
        file.close();
